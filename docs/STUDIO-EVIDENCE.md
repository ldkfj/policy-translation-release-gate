...
- Four objections are recorded; the current exact-release external-wallet objection is ID `4` against candidate `16`, with the independent observer address and reason intact. Candidate 16 has three objections in its paginated readback.
...
Project source is public at `https://github.com/ldkfj/policy-translation-release-gate`. The verified production frontend is deployment `dpl_AcLC5FenPms81vhyZwh1eJTdG9Dh` at `https://policy-translation-release-gate.vercel.app`. Primary browser smoke and current exact-release external-wallet objection/readback passed; user confirmation of reload/disconnect remains pending, followed by final reviewer approval, submission, and experience-ledger update.
